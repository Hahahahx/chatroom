...
import {reqGetGroup, reqGetUserByPhone, reqReportToUser} from "../../../api";
const { TreeNode } = Tree;
const { Meta } = Card;
...
class ReportGroup extends Component{
    ...
    handleSubmit=async ()=>{
        const {group,textMsg,imgMsg,value,text}=this.state;
    ...
    async componentDidMount() {
        ...
        const response = await reqGetGroup(group);
        if(response.data.data){
            const group = response.data.data.group;
            this.setState({group});
        }else{
            message.error("未获取到信息",2).then(i=>{
                this.props.history.goBack()
            })
        }
    }
    ...
    render() {
        ...
        let talkList = talking.length!==0?talking.filter(item=>parseInt(item.touser) === parseInt(groupId)):[];
            ...
            <Layout>
                <ScrollContent>
                    ...
                    <Card style={{ width: '100%', marginTop: 16}}>
                        <Meta
                            avatar={
                                <Avatar src={group.groupHeader} size={40} />
                            }
                            title={group.groupName}
                            description={group.groupBrief}
                        />
                    </Card>
                    ...
                    <Tree
                        ...
                        {
                            talkList.filter(talk=>parseInt(talk.user)!==parseInt(currentUser.userPhone)&&!talk.typeIsImage)
                                .map(talk=>{
                                    if(!talk.typeIsImage){
                                        return (
                                            <TreeNode key={`(${talk.user})${talk.sendTime}：${talk.message}`} title={`(${talk.user})${talk.sendTime}：${talk.message}`}/>
                                        );
                                    }
                                })
                        }
...
export default connect(mapStateToProps, mapDispatchToProps)(ReportGroup)