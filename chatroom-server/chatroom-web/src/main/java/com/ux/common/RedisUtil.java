...
public final class RedisUtil {
     ...
     * @param key 建
     * @param value 值
     * @param time 时间（秒） time要大于0 如果小于等于0 则将时间设为永久
     * @return true成功 false失败
     * */
     ...
     * @return 最后一个键值
     * */
    public Map.Entry<Object,Object> hashMapgetlast(String key){
        List<Map.Entry<Object,Object>> list = new ArrayList<Map.Entry<Object,Object>>();
        if (!(hashMapget(key) != null)){
            return null;
        }
        list.addAll(hashMapget(key).entrySet());
        if(list.size()>0){
    ...
    public long listGetListSize(String key) {
        try {
            return redisTemplate.opsForList().size(key);            //————————————可能空指针！！！
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
        }
